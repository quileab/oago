...
<head>
    ...
    <title>Sitio en Mantenimiento - {{ config('app.name') }}</title>
...
<body class="antialiased overflow-hidden">
    ...
    <div class="fixed inset-0 -z-20 opacity-60">
        <div class="w-full h-full bg-cover bg-center blur-3xl" 
             style="background-image: url('{{ asset('imgs/rsrc/503_bg.png') }}');">
        </div>
    </div>
    ...
    <main class="min-h-screen flex flex-col justify-between p-2 md:p-4 relative z-10">
        ...
        <div class="flex justify-center w-full">
            <div class="max-w-xl w-full bg-base-100/60 backdrop-blur-xl p-4 md:p-6 rounded-3xl shadow-2xl border border-white/10 text-center">
                <div class="relative inline-block mb-1">
                    <h1 class="text-6xl font-black text-warning/40 select-none">503</h1>
                </div>
                <h2 class="text-2xl font-black uppercase tracking-tighter mb-1 text-white drop-shadow-md">Volvemos Pronto</h2>
                <p class="text-white/90 font-bold drop-shadow-sm text-sm">Estamos realizando tareas de mantenimiento para mejorar tu experiencia.</p>
            </div>
        </div>
        ...
        <div class="flex justify-center w-full mb-4">
            <div class="max-w-xs w-full drop-shadow-2xl">
                <x-button label="COMPROBAR NUEVAMENTE" icon="o-arrow-path" class="btn-primary w-full font-bold shadow-2xl" onclick="window.location.reload()" />
            </div>
        </div>
