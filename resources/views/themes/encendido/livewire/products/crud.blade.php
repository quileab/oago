...
new class extends Component
{
    ...
    {
        $this->assertTempStorageWritable();
    ...
    private function uploadStorageError(Throwable $e): void
    {
        Log::error('Fallo al procesar archivo temporal de Livewire: '.$e->getMessage());

        $this->error('No se pudo procesar la imagen. Verificá que el directorio storage/app/private/livewire-tmp exista y tenga permisos de escritura en el servidor.');
    }

    private function assertTempStorageWritable(): void
    {
        $storage = FileUploadConfiguration::storage();
        $directory = FileUploadConfiguration::directory();

        rescue(
            fn () => is_dir($storage->path($directory))
                ? null
        ...
        if (! is_dir($path) || ! is_writable($path)) {
            Log::warning("Almacenamiento temporal de Livewire no escribible en {$path}");

            $this->error('El almacenamiento temporal de imágenes (storage/app/private/livewire-tmp) no es escribible en el servidor. Corregí los permisos para poder subir imágenes.');
        }
    }
                        ...
                        <div class="flex flex-col items-center mb-6" 
                            ...
                            <x-file wire:model.live="photo" accept="image/*" x-on:change="checkSize">
                                ...
                                @php
                                    ...
                                    if (! $previewUrl) {
                                        $previewUrl = (isset($formData['image_url']) && $formData['image_url']) ? $formData['image_url'] : '/imgs/Logos/ER50.png';
                                    } else {
