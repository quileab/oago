...
<div>
    <x-card title="Detalle de la Compra {{ Session::has('updateOrder') ? '#' . Session::get('updateOrder') : '' }}"
            ...
            <tbody>
                @foreach ($cart_content as $item)
                    <tr class="odd:bg-white/[0.03]">
                        ...
                        <td class="text-right font-bold text-emerald-400">${{ number_format($item['subtotal'], 2) }}</td>
                    </tr>
                @endforeach
            </tbody>
            <tfoot class="border-b bg-white/5">
                <tr>
        ...
        <x-form wire:submit.prevent="save">
            ...
            <x-slot:actions>
                <x-button wire:click="save" icon="o-check" class="btn-primary w-full md:w-auto px-12 magnetic hover:shadow-[0_0_25px_rgba(220,38,38,0.6)] hover:scale-105 transition-all duration-300 font-black tracking-wide" type="submit" 
